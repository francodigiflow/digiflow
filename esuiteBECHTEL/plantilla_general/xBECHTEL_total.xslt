...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:pe="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
	...
	<xsl:template name="Total">
		<table border="0" width="100%">
			<tr>
				...
				<td width="30%">
					<table rules="rows" border="1" width="100%" bordercolor="#000000" cellpadding="1" cellspacing="0">
						<!--	<tr>-->
						<!--<td width="40%">
							...
							<td width="15%">
								<font face="Arial, Helvetica, sans-serif" size="2">
									<strong>: <xsl:call-template name="CTSTrullinQuiereElSimbolo" /> </strong>
								</font>
							</td>
						...
						<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe1:DatosAdicionales/pe1:DatoAdicional">
							...
							<xsl:value-of select="pe1:Valor" /></xsl:if>
							</xsl:if>
						</xsl:for-each>
						...
						<xsl:if test="//cac:TaxSubtotal/cac:TaxCategory/cbc:TaxExemptionReasonCode='40' and //cbc:UBLVersionID='2.1'">
							<tr>
								<td width="40%">
									<font face="Arial, Helvetica, sans-serif" size="2">
										<strong>Total Exportacion</strong>
									</font>
								</td>
								<td width="10%">
									<font face="Arial, Helvetica, sans-serif" size="2">
						...
						<tr>
							...
							<td width="50%" align="right">
								<font face="Arial, Helvetica, sans-serif" size="2">
									<!--<xsl:if test="/pe:Invoice/cbc:InvoiceTypeCode = '01' or /pe1:CreditNote or /pe2:DebitNote">
								...
								<xsl:if test="cbc:TaxAmount!='0.00'">
									<xsl:value-of select="format-number(cbc:TaxAmount, '###,###,##0.00', 'pen')"/>
										...
										<xsl:variable name="IGV">
											<xsl:for-each select="/pe:Invoice/cac:TaxTotal">
												<xsl:if test="//cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:ID= '1000'">
													<!--<xsl:call-template name="SimboloMoneda"/> &#xA0;<xsl:value-of select="format-number(//cac:TaxSubtotal/cbc:TaxAmount, '###,###,##0.00', 'pen')"/>&#xA0;-->
												</xsl:if>
											</xsl:for-each>
	...
	<xsl:template name="son-pesos">
		<table border="0" width="100%" cellpadding="1" cellspacing="0" bordercolor="#000000">
			<tr>
				<td width="100%">
					<font face="Arial, Helvetica, sans-serif" size="2">
						<b>SON :</b>
					...
					<font face="Arial, Helvetica, sans-serif" size="2">
						<xsl:if test="//cbc:UBLVersionID='2.0'">
							...
							<xsl:if test="$ValorDate &lt; '20161231'">
							...
							<xsl:if test="$ValorDate &gt; '20170101'">
			...
			<xsl:if test="/pe1:CreditNote or /pe2:DebitNote">
				<tr>
					<td width="100%" align="left">
						<font color="#000000" size="2" face="Arial, Helvetica, sans-serif">SUSTENTO: <xsl:value-of select="//cac:DiscrepancyResponse/cbc:Description"/>
							<br/>DOCUMENTO(S) DE REFERENCIA:</font>
					</td>
				</tr>
				<tr>
					<td>
	...
	<xsl:template name="DatoDetraccion">
		<xsl:choose>
	...
	<xsl:template name="CTSOtraTonteria">
		<!--<table rules="none" border="0" width="54%" bordercolor="#000000" cellpadding="1" cellspacing="0">-->
		<table border="0" width="54%" cellpadding="1" cellspacing="0" bordercolor="#000000">
			<tr>
				<td>
					<font face="Arial, Helvetica, sans-serif" size="2">
						<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe:DatosAdicionales/pe:DatoAdicional">
			...
			<xsl:variable name="ValorDate">
				...
				<xsl:variable name="varvar">
					...
					<xsl:if test="$ValorDate &lt; '20161231'">
						...
						<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe1:DatosAdicionales/pe1:DatoAdicional">
							<xsl:if test="pe1:Codigo = '7' ">
								<xsl:value-of select="pe1:Valor" />|
							</xsl:if>
						</xsl:for-each>
						<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe2:DatosAdicionales/pe2:DatoAdicional">
							<xsl:if test="pe2:Codigo = '7'">
								<xsl:value-of select="pe2:Valor" />|
							</xsl:if>
						</xsl:for-each>	
<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe:DatosAdicionales/pe:DatoAdicional">
								<xsl:if test="pe:Codigo = '7' "><xsl:value-of select="pe:Valor" />|</xsl:if>
							</xsl:for-each>
							<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe1:DatosAdicionales/pe1:DatoAdicional">
								<xsl:if test="pe1:Codigo = '7' "><xsl:value-of select="pe1:Valor" />|</xsl:if>
							</xsl:for-each>
							<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe2:DatosAdicionales/pe2:DatoAdicional">
								<xsl:if test="pe2:Codigo = '7' "><xsl:value-of select="pe2:Valor" />|</xsl:if>
							</xsl:for-each>						
					...
					<xsl:if test="$ValorDate &gt; '20170101'">
							...
							<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe:DatosAdicionales/pe:DatoAdicional">
								<xsl:if test="pe:Codigo = '4' "><xsl:value-of select="pe:Valor" /></xsl:if>
							</xsl:for-each>
							<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe1:DatosAdicionales/pe1:DatoAdicional">
								<xsl:if test="pe1:Codigo = '4' "><xsl:value-of select="pe1:Valor" /></xsl:if>
							</xsl:for-each>
							<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe2:DatosAdicionales/pe2:DatoAdicional">
								<xsl:if test="pe2:Codigo = '4' "><xsl:value-of select="pe2:Valor" /></xsl:if>
							</xsl:for-each>
					</xsl:if>
				</xsl:variable>
