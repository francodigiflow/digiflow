...
<!--
		...
		<xsl:choose>
			...
			<xsl:otherwise>		
			<xsl:if test="//cac:PaymentTerms/cbc:PaymentMeansID ='Credito'"> CREDITO -
			<xsl:call-template name="DatoAdicional01"> <xsl:with-param name="var" select="06" /></xsl:call-template>
			</xsl:if> 

			<xsl:if test="//cac:PaymentTerms/cbc:PaymentMeansID ='Contado'"> CONTADO -
			<xsl:call-template name="DatoAdicional01"> <xsl:with-param name="var" select="06" /></xsl:call-template>
			</xsl:if>			
			</xsl:otherwise>
