		...
		<table border="0" cellpadding="1" cellspacing="1" width="100%" rules="none">
				...
				<td width="60%">
					<table border="0" cellSpacing="0" borderColor="#FFFFFF" cellPadding="1" width="100%" rules="none"><tbody>
						...
						<xsl:if test="substring-before($tcurva,'|')!=''" >																																																		
							...
							<br/><xsl:value-of select="substring-before(substring-after($tcurva,'|'),'|')"/>											
							<xsl:if test="substring-before(substring-after(substring-after($tcurva,'|'),'|'),'|')!=''">
							<br/><xsl:value-of select="substring-before(substring-after(substring-after($tcurva,'|'),'|'),'|')"/>
							<br/><xsl:value-of select="substring-before(substring-after(substring-after(substring-after($tcurva,'|'),'|'),'|'),'|')"/>
							<xsl:if test="substring-before(substring-after(substring-after(substring-after(substring-after($tcurva,'|'),'|'),'|'),'|'),'|')!=''">
							<br/><xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after($tcurva,'|'),'|'),'|'),'|'),'|')"/>
							<br/><xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(substring-after($tcurva,'|'),'|'),'|'),'|'),'|'),'|')"/>
							<br/><xsl:value-of select="substring-before(substring-after(substring-after(substring-after(substring-after(substring-after(substring-after($tcurva,'|'),'|'),'|'),'|'),'|'),'|'),'|')"/>
							</xsl:if>
							</xsl:if>
						</xsl:if>
						<xsl:if test="substring-before($tcurva,'|')=''" >
							<br/><xsl:value-of select="$tcurva"/>		
						</xsl:if>
						</font> 
			...
			<xsl:variable name="spot">
					<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe:DatosAdicionales/pe:DatoAdicional">
						<xsl:if test="pe:Codigo = '09'"><xsl:value-of select="pe:Valor" /></xsl:if>
					</xsl:for-each>
					<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe1:DatosAdicionales/pe1:DatoAdicional">
						<xsl:if test="pe1:Codigo = '09'"><xsl:value-of select="pe1:Valor" /></xsl:if>
					</xsl:for-each>
					<xsl:for-each select="//ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/pe2:DatosAdicionales/pe2:DatoAdicional">
						<xsl:if test="pe2:Codigo = '09'"><xsl:value-of select="pe2:Valor" /></xsl:if>
					</xsl:for-each>	
				</xsl:variable>
				...
				<td>
					<font face="Arial, Helvetica, sans-serif" size="1"> 
						...
						<br/>con el gobierno central D-Leg. N°90 G/N Nación %12.00 Cod Serv. 022
					</font>
				</td>
				</tr>
				</xsl:if>
			...
			<tr>
				<td width="100%">
					<table border="0" cellSpacing="0" borderColor="#FFFFFF" cellPadding="1" width="100%" rules="none"><tbody>
						<tr>
							<td width="50%">
								<table border="1" cellSpacing="0" borderColor="#000000" cellPadding="1" width="100%"><tbody>
									<tr><td align="center" >

										<font face="Arial, Helvetica, sans-serif" size="1"> 
										<br/>
										SI LA BOLETA NO ES CANCELADA EN LA
