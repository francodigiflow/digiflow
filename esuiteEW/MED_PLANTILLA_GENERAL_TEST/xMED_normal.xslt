...
<xsl:stylesheet version="1.0" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:pe="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
	...
	<xsl:include href="xMED_encab.xslt"/>
   		<xsl:include href="xMED_detalle.xslt"/>
   		<xsl:include href="xMED_bottom.xslt"/>
	 	<xsl:include href="xMED_datos.xslt"/>
		<xsl:include href="xMED_total.xslt"/>
	<xsl:template match="/">
		<html xmlns:sbc="urn:oasis:names:specification:ubl:schema:xsd:SignatureBasicComponents-2" xmlns:sac="urn:oasis:names:specification:ubl:schema:xsd:SignatureAggregateComponents-2"
			...
			<xsl:for-each select="/pe:Invoice/cac:InvoiceLine">
				...
				<!--Variable para obtener el string 26;27;.....50;-->
				<!--<xsl:variable name="FL2">
			<xsl:for-each select="/pe:Invoice/cac:InvoiceLine">
			...
			<xsl:for-each select="/pe:Invoice/cac:InvoiceLine">
				...
				<!--Variable para tomar el ultimo valor del Foreach-->
				<!--<xsl:variable name="LI">
			<xsl:value-of select="substring-before(substring-after($FL,'24;'),';')"/>
		</xsl:variable>-->
				<!--Variable para tomar el ultimo valor del Foreach-->
				<!--<xsl:variable name="LI2">
			<xsl:value-of select="substring-before(substring-after($FL2,'49;'),';')"/>
		</xsl:variable>-->
				<!--Variable para tomar el ultimo valor del Foreach-->
				<!--<xsl:variable name="LI3">
			<xsl:value-of select="substring-before(substring-after($FL3,'74;'),';')"/>
		</xsl:variable>-->
