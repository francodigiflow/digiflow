				...
				<td>
					<font face="Arial, Helvetica, sans-serif" size="1">
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<!--<xsl:if test="//cbc:Note[@languageLocaleID='1002'] and //cac:TaxCategory/cbc:TaxExemptionReasonCode!='12' or //cac:TaxCategory/cbc:TaxExemptionReasonCode!='32'">
		...
		<table border="1" width="100%" cellpadding="1" cellspacing="0" bordercolor="#000000" heigth="100%">
			...
			<td align="right">
				...
				<xsl:if test="//cbc:UBLVersionID='2.1'">
							...
							<!--<xsl:value-of select="$Exonerado"/>-->
							<xsl:if test="$Exportacion = ''">0.00&#xA0;</xsl:if>
						</xsl:if>
			...
			<tr>
				...
				<td align="right">
					<font face="Arial, Helvetica, sans-serif" size="1">
						...
						<xsl:if test="//cbc:UBLVersionID='2.1'">
							<xsl:choose>
								<xsl:when test="//cac:AllowanceCharge/cbc:Amount!='' and //cac:AllowanceCharge/cbc:AllowanceChargeReasonCode='02'">
									<xsl:value-of select="format-number(//cac:AllowanceCharge/cbc:Amount,'###,###,##0.00','pen')"/></xsl:when>
									<xsl:when test="//cac:AllowanceCharge/cbc:Amount!='' and //cac:AllowanceCharge/cbc:AllowanceChargeReasonCode='03'">
									<xsl:value-of select="format-number(//cac:AllowanceCharge/cbc:Amount,'###,###,##0.00','pen')"/></xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="format-number('0.00','###,###,##0.00','pen')"/>&#xA0;</xsl:otherwise>
							</xsl:choose>
							...
							<xsl:if test="//cbc:UBLVersionID='2.1'">
								...
								<!--<xsl:choose>
									<xsl:when test="//cbc:Note[@languageLocaleID='1002']">
										<xsl:value-of select="$IGVGRAT"/>&#xA0;</xsl:when>
			...
			<xsl:for-each select="/pe2:DebitNote/cac:TaxTotal">
				<tr>
					<td bgcolor="#C4BD97">
						<font face="Arial, Helvetica, sans-serif" size="1">
							<strong>Total
								<!-- <xsl:value-of select="cac:TaxSubtotal/cac:TaxCategory/cac:TaxScheme/cbc:Name"/> -->IGV 18%</strong>
						</font>
					</td>
					<td align="right">
						<font face="Arial, Helvetica, sans-serif" size="1">
