...
<xsl:stylesheet version="1.0" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:pe="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2"
	...
	<xsl:variable name="numColB2">
		<xsl:if test="/pe:Invoice!=''">
			<xsl:value-of select="50-count(/pe:Invoice/cac:InvoiceLine)"/>
		</xsl:if>
		<xsl:if test="/pe1:CreditNote!=''">
			<xsl:value-of select="50-count(/pe1:CreditNote/cac:CreditNoteLine)"/>
		</xsl:if>
		<xsl:if test="/pe2:DebitNote!=''">
			<xsl:value-of select="50-count(/pe2:DebitNote/cac:DebitNoteLine)"/>
		</xsl:if>
	</xsl:variable>

	<xsl:variable name="numColB3">
		<xsl:if test="/pe:Invoice!=''">
			<xsl:value-of select="75-count(/pe:Invoice/cac:InvoiceLine)"/>
		</xsl:if>
		<xsl:if test="/pe1:CreditNote!=''">
			<xsl:value-of select="75-count(/pe1:CreditNote/cac:CreditNoteLine)"/>
		</xsl:if>
		<xsl:if test="/pe2:DebitNote!=''">
			<xsl:value-of select="75-count(/pe2:DebitNote/cac:DebitNoteLine)"/>
		</xsl:if>
	</xsl:variable>

	<xsl:variable name="numColB4">
		<xsl:if test="/pe:Invoice!=''">
			<xsl:value-of select="101-count(/pe:Invoice/cac:InvoiceLine)"/>
		</xsl:if>
		<xsl:if test="/pe1:CreditNote!=''">
			<xsl:value-of select="101-count(/pe1:CreditNote/cac:CreditNoteLine)"/>
		</xsl:if>
		<xsl:if test="/pe2:DebitNote!=''">
			<xsl:value-of select="101-count(/pe2:DebitNote/cac:DebitNoteLine)"/>
		</xsl:if>
	</xsl:variable>

	<xsl:variable name="TotItems">
		<xsl:if test="/pe:Invoice!=''">
	...
	<xsl:include href="xELEDE_encab.xslt"/>
	<xsl:include href="xELEDE_datos.xslt"/>
	<xsl:include href="xELEDE_detalle.xslt"/>
	<xsl:include href="xELEDE_bottom.xslt"/>
	<xsl:include href="xELEDE_total.xslt"/>
	...
	<xsl:template match="/">
		<html xmlns:sbc="urn:oasis:names:specification:ubl:schema:xsd:SignatureBasicComponents-2" xmlns:sac="urn:oasis:names:specification:ubl:schema:xsd:SignatureAggregateComponents-2"
			...
			<body>
				...
				<xsl:if test="$FF &gt;'23' or $FC &gt;'23' or $FD &gt;'23'">
							...
							<tbody>
								...
								<tr>
									<td>
										<xsl:choose>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='01'">
												<xsl:call-template name="Detalle2"/>
											</xsl:when>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='03'">
												<xsl:call-template name="Detalle2"/>
											</xsl:when>
				...
				<xsl:if test="$FF &gt;'46' or $FC &gt;'46' or $FD &gt;'46'">
					<!--Función para realizar el salto de lÃ­nea-->
					<div class="saltar-pagina">
							...
							<tbody>
								...
								<tr>
									<td>
										<xsl:choose>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='01'">
												<xsl:call-template name="Detalle3"/>
											</xsl:when>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='03'">
												<xsl:call-template name="Detalle3"/>
											</xsl:when>
				...
				<xsl:if test="$FF &gt;'69' or $FC &gt;'69' or $FD &gt;'69'">
					<!--Función para realizar el salto de lÃ­nea-->
					<div class="saltar-pagina">
						<!--<xsl:value-of select="$LI3"/>-->&#xA0;</div>
							...
							<tbody>
								...
								<tr>
									<td>
										<xsl:choose>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='01'">
												<xsl:call-template name="Detalle4"/>
											</xsl:when>
											<xsl:when test="/pe:Invoice/cbc:InvoiceTypeCode='03'">
												<xsl:call-template name="Detalle4"/>
											</xsl:when>
