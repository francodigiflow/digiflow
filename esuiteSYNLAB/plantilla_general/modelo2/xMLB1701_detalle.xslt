		...
		<table border="0" width="100%" cellpadding="0" cellspacing="2">
			...
			<xsl:variable name="min">
				<xsl:choose>
					...
					<xsl:when test="$pagina = '2'">43</xsl:when>
					<xsl:when test="$pagina = '3'">86</xsl:when>
					<xsl:when test="$pagina = '4'">129</xsl:when>
					<xsl:when test="$pagina = '5'">172</xsl:when>
					<xsl:when test="$pagina = '6'">215</xsl:when>
					<xsl:otherwise></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="max">
				<xsl:choose>					
					<xsl:when test="$pagina = '1'">44</xsl:when>
					<xsl:when test="$pagina = '2'">87</xsl:when>
					<xsl:when test="$pagina = '3'">130</xsl:when>
					<xsl:when test="$pagina = '4'">173</xsl:when>
					<xsl:when test="$pagina = '5'">216</xsl:when>
					<xsl:when test="$pagina = '6'">259</xsl:when>
					<xsl:otherwise></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
