...
<xsl:stylesheet version="1.0" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" 
	...
	<xsl:variable name="v_StatusMultiPagina"><xsl:value-of select="'S'"/></xsl:variable>
	
	<xsl:include href="xGRELAPASA_encab.xslt"/>
	<xsl:include href="xGRELAPASA_detalle.xslt"/>
	<xsl:include href="xGRELAPASA_bottom.xslt"/>
	<xsl:include href="xGRELAPASA_datos.xslt"/>
	<xsl:include href="xGRELAPASA_total.xslt"/>

	<xsl:variable name="v_ItemPorPagina1"><xsl:value-of select="13"/></xsl:variable>
	<xsl:variable name="v_ItemPorPagina"><xsl:value-of select="30"/></xsl:variable>
	
	
	<xsl:variable name="varPagRestoSeccionesFinales"><xsl:value-of select="20 div $v_ItemPorPagina"/></xsl:variable>
			...
			<body>
				<xsl:if test="$v_StatusMultiPagina='S'">				
					<xsl:call-template name="tmpSiMultipagina">
						...
						<xsl:with-param name="ContadorItem" select="1"/> <!-- NRO DE PAG. -->
					</xsl:call-template>
				</xsl:if>
...
<xsl:template name="tmpSiMultipagina">
		<xsl:param name="cont"/> <!-- caNT. TOTAL DE PAG -->
		<xsl:param name="ContadorItem"/> <!-- nRO DE PAG ACT --> 
				...
				<table cellSpacing="0" width="100%" border="0">
					...
					<tbody>
					  <div class="content">
					  	<tr><td>&#xA0;</td></tr>
						<tr><td>&#xA0;</td></tr>
<!-- CABECERA -->		<tr><td><xsl:call-template name="Encabezado"/></td></tr>
					<xsl:if test="$ContadorItem = 1">
<!-- DATOS CLIENTE -->	<tr><td><xsl:call-template name="Datos_Puntos"/></td></tr>
						<tr><td><xsl:call-template name="DatosReceptor"/></td></tr>
						<tr><td><xsl:call-template name="Cuadro_Conductor_Transporte"/></td></tr>
						<tr><td><xsl:call-template name="TipoComprobante"/></td></tr>
						<tr><td><xsl:call-template name="MotivoTraslado"/></td></tr>
					</xsl:if>
<!-- DETALLE -->		<tr>
							<td>
							<!--<xsl:if test="$ContadorItem = 1">-->
...
<!-- TOTALES -->		<tr><td>
	...
	<xsl:variable name="totalpag">				
		...
		<xsl:if test="($numitems) &gt;'13'  and ($numitems) &lt;'39' ">
